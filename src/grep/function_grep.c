...
void print_lines_found(int argc, char **argv, int position,
  ...
  int capacity, regexeced, flag_for_l, files_count, count, matched_in_string,
      flag_c_was_printed, argv_was_printed;
  char *line_from_file;
      ...
      for (int i = 0; i < *max_number_strings; i++) {
        ...
        if (matched_in_string == 1) {
          if (files_count > 0 && flags->l == 0 && flags->h == 0) {
            if ((flags->c > 0 && argv_was_printed == 0) || flags->c == 0) {
              printf("%s:", argv[position]); /*e f v i l*/
              argv_was_printed = 1;
            }
          }
          print_l_flag(argv, position, &flag_for_l, flags); /*c n o*/
          print_c_flag(line_from_file, flags, count, i, max_number_strings,
                       &flag_c_was_printed);
          // print_o_flag(line_from_file, regexeced, regexed, flags, i);
...
//   regmatch_t pmatch;
//   unsigned start_to_search = 0;
//   int regexeced, number_of_string = iteration + 1;

//   if (flags->o > 0 && res_regexec == 0) {
//     while (start_to_search < strlen(line)-1) {
//       regexec(&regexed, line + start_to_search, 1, &pmatch, 0);
//       if (flags->n > 0) {
//         printf("%d:", number_of_string);
//       }
//       for (pmatch.rm_so; pmatch.rm_so < pmatch.rm_eo; pmatch.rm_so++) {
